...
class AptClient(EventsObject):
    ...
    def upgrade_package(self, package_name):
        return self._queue_task("remove","install", package_name = package_name)
    ...
    def get_progress_info(self):
        ...
        if self._running:
            ...
            res["tasks"].append({"role": self._apt_thread.task_type, "status": status, "progress": task_progress, "task_id": self._apt_thread.task_id, "task_params": self._apt_thread.params, "cancellable": True})
        else:
            if total_nb_tasks > 0:
                task_perc = 100. / total_nb_tasks
